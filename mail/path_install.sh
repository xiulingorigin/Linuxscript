...
echo "account xiulingorigin@163.com" >> /etc/msmtprc #发信账号
echo "host smtp.163.com" >> /etc/msmtprc  #SMTP邮件服务器地址
echo "from xiulingorigin@163.com" >> /etc/msmtprc  #发件人地址
echo "auth login" >> /etc/msmtprc
echo "user xiulingorigin@163.com" >> /etc/msmtprc  #邮件服务器登录账号
echo "password ****" >> /etc/msmtprc  #邮件服务器登录密码
echo "account default:xiuling_permeate" >> /etc/msmtprc  #默认账号名称

chmod 600 /etc/msmtprc

echo "set sendmail='/usr/bin/msmtp'" >> /etc/Muttrc  #设置msmtp命令路径
echo "set use_from=yes" >> /etc/Muttrc
echo "set realname='xiuling'" >> /etc/Muttrc  #设置发件人的别名，可以不是邮件地址
echo "set from='xiulingorigin@163.com'" >> /etc/Muttrc  #设置发件人地址
echo "set envlope_from=yes" >> /etc/Muttrc
